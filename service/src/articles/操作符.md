...
js 中的数都是以 64 位格式 存储，但是所有的位运算都是转化成 32 位整数然后进行操作，完成后再转换成 64 位格式
...
- \> >= < <=
  - 只有两个都是字符串时才会转化成 unicode 字符然后比较
