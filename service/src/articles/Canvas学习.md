...
![image-20211008235158686](D:\VsCode\web\blog\service\src\articles\upload\image-20211008235158686.png)

## 1.快速开始

### 1.1 获取 canvas 上下文,我们所有绘图操作都是在上下文操作中完成的
...
![image-20211008180921563](C:\Users\25760\AppData\Roaming\Typora\typora-user-images\image-20211008180921563.png)
...
![image-20211008180852920](C:\Users\25760\AppData\Roaming\Typora\typora-user-images\image-20211008180852920.png)

### 通过上面的介绍我们大致了解了 Canvas 如何进行基本的绘制，接下来让我们开始这个小项目 😏

## 2 开始项目

### 2.1 生成一个小球

一个小球应该是有很多属性的，如果我们写死,可以使用一个对象直接表示，但是表示多个不同的小球很不方便，所以可以编写一个构造函数 Ball 通过 new 获得一个小球对象实例
...
给小球添加一个方法 draw,通过小球位置和半径在 canvas 中绘制
...
给小球增加一个方法 move 改变小球的位置即可使小球动起来,并且移动的时候还要判断是否到了 canvas 的边界
...
小球的函数写好了,怎么使小球流畅的动起来呢？这里使用一下浏览器提供的 API, window.requestAnimationFrame()
...
![ball](C:\Users\25760\OneDrive\桌面\ball.gif)
...
项目的函数基本介绍完成，但是实际使用时也发现了一些问题，例如生成小球的时候可能生成位置还有其他的小球的 bug,后面给生成函数加了一个判断函数可以解决这个问题。后面也进行了一些优化,将几个移动的函数加到函数的原型上以及一些函数
...
[canvas API 中文网 - Canvas API 中文文档首页地图](https://www.canvasapi.cn/)
